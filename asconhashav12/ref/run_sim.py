...
c_source_files = ["main.c", "hash.c"]
optimization_levels = ["O0", "O1", "O2", "Os", "O3", "Og"]
output_folders = ["sim_O0", "sim_O1", "sim_O2", "sim_Os", "sim_O3", "sim_Og"]
...
# Define cycle costs for different instructions (these are arbitrary examples)
instruction_cycle_costs = {
    "add": 1,
    "mul": 2,
    "sub": 1,
    "div": 3,
    "c.swsp": 1,  # Example for compressed store instruction
    # Add other instructions and their cycle costs here
}
...
for idx, opt_level in enumerate(optimization_levels):
    ...
    # Estimate total clock cycles, defaulting to 1 cycle for unspecified instructions
    total_cycles = sum(instruction_frequency[opcode] * instruction_cycle_costs.get(opcode, 1) for opcode in instruction_frequency)
    ...
    with open(f"{output_folders[idx]}/stats.txt", "w") as stats_file:
        ...
        stats_file.write(f"Estimated Total Clock Cycles: {total_cycles}\n")

...
void system_init(void) {
  ...
  if (install_exception_handler(UART_IRQ_NUM, &uart_irq_handler)) {
    puts("Failed to install UART exception handler\n");
    return;
  }
  if (install_exception_handler(TIMER_IRQ_NUM, &timer_irq_handler)) {
    puts("Failed to install Timer exception handler\n");
    return;
  }

  // Enable interrupts
  uart_enable_rx_int();
  set_global_interrupt_enable(1); // Enable global interrupts
}
